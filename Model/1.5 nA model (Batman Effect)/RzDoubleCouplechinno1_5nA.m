...
gKFactor = 70;     % default: 57
gAFactor = 165;    % default: 166
gNFactor = 500;    % default: 650
gLFactor = 12;     % default: 12
gHFactor = 2;      % default: 1

% Compartment Conductance Parameters
gCompFactor = 40;  % default: 55
gGCoupFactor = 22.42; % default: [21:22]
...
c1_factor = 110; % Soma Capacitance
c2_factor = 10;  % Gap Junction Capacitance
c3_factor = 20;  % Spike Initation Zone Capacitance

% calling Rz cell model
gGCoupFactor_vals = [22.48];
...
for Gi = 1:length(gGCoupFactor_vals)
    ...
    [v1_Pre,v2_Pre,v3_Pre,v1_Post,v2_Post,v3_Post] = DoubleRz_MultiComp(IinjModel,dt, gCompFactor, gGCoupFactor, gNFactor, gKFactor, gAFactor, gLFactor, gHFactor, EL, EH, EN, EK, c1_factor, c2_factor, c3_factor); % call the model
...
%    Analyzing
    ...
    resting_potential_pre_stim_Pre = mean(v1_Pre(stim_onset-window_length:stim_onset));
    resting_potential_pre_stim_Post = mean(v1_Post(stim_onset-window_length:stim_onset));
    ...
    amplitude_Pre = resting_potential_pre_stim_Pre - hyperpolarized_membrane_potential_Pre;
    amplitude_Post = resting_potential_pre_stim_Post - hyperpolarized_membrane_potential_Post;
...
batmansave.v1_Pre = v1_Pre;
batmansave.v1_Post = v1_Post;
    ...
    % % Input Current
    % subplot(3,2,1); cla; hold on; 
    % plot(t,IinjModel/1000,'-',color=c1);
    % plot(t,IinjReal/1000,'-',color=c2);
    % 
    % xlim([0,Tall]); set(gca,'xtick',0:0.5:Tall);
    % ylim([-1.5,2.0]); set(gca,'ytick',-3:1:3);
    % xlabel('time [s]'); ylabel('current [nA]'); 
    % title('Applied Stimulus to Soma of Presynaptic cell')
    % 
    % 
    % % Presynaptic compartment 1 (soma = non-spiking)
    % subplot(3,2,3); cla; hold on; 
    ...
    % % Postsynaptic compartment 1 (soma = non-spiking)
    % subplot(3,2,4); cla; hold on; 
    % plot(t,v1_Post,'-',color=c1); 
    % txt_post = "Spike count: " + num2str(spike_count_Post);
    % text(0.5,40,  txt_post, color=c1, fontsize=10);
    % txt_lat_post = "Spike Latency: " + num2str(spike_latency_Post);
    % text(0.5,31,  txt_lat_post, color=c1, fontsize=10);
    % txt_hyper_post = "Hyper. Amp.: " + num2str(amplitude_Post);
    % text(0.5,22,  txt_hyper_post, color=c1, fontsize=10);
    % txt_isi_post = "ISI: " + num2str(inter_spike_interval_Post);
    % text(0.5,13,  txt_isi_post, color=c1, fontsize=10);
    % xlim([0,Tall]); set(gca,'xtick',0:0.5:Tall);
    % ylim([-105,45]); set(gca,'ytick',-100:20:40);
    % xlabel('time [s]'); ylabel('potential [mV]'); 
    ...
    % Vreal_pre_window = [pre_cell_11(9, 174001:199000), pre_cell_11(9, 239001:264000)];
    % 
    % % Presynaptic cell Soma (soma - Actual Cell)
    % subplot(3,2,5); cla; hold on; 
    % plot(t,Vreal_window,'-',color=c2);
    % xlim([0,Tall]); set(gca,'xtick',0:0.5:Tall);
    % ylim([-105,45]); set(gca,'ytick',-100:20:40);
    % xlabel('time [s]'); ylabel('potential [mV]'); 
    ...
    % 
    % % Concatenate the data if you want to plot it as one continuous plot
    % Vreal_post_window = [post_cell_11(9, 174001:199000), post_cell_11(9, 239001:264000)];
    % 
    % % Presynaptic cell Soma (soma - Actual Cell)    
    % subplot(3,2,6); cla; hold on; 
    % plot(t,Vreal_post_window,'-',color=c2);
    % xlim([0,Tall]); set(gca,'xtick',0:0.5:Tall);
    % ylim([-105,45]); set(gca,'ytick',-100:20:40);
    % xlabel('time [s]'); ylabel('potential [mV]'); 
    % title('Membrane Potential of Presynaptic cell Soma (Real)')
...
%% Save 
    % Amplitude
    PreModelCellFeatures.amplitude = amplitude_Pre;
    PostModelCellFeatures.amplitude = amplitude_Post;
    
    % Spike count
    PreModelCellFeatures.spike_count = spike_count_Pre;
    PostModelCellFeatures.spike_count = spike_count_Post;
    
    % Spike latency
    PreModelCellFeatures.spike_latency = spike_latency_Pre;
    PostModelCellFeatures.spike_latency = spike_latency_Post;
    
    % Inter-Spike Interval
    PreModelCellFeatures.inter_spike_interval = inter_spike_interval_Pre;
    PostModelCellFeatures.inter_spike_interval = inter_spike_interval_Post;

%% mathematical comparrison of pre and post cells (ratio/ difference)
    % Pre and post
    CompareModelCells.amplitude = PostModelCellFeatures.amplitude ./ PreModelCellFeatures.amplitude;
    CompareModelCells.spike_count = PostModelCellFeatures.spike_count ./ PreModelCellFeatures.spike_count;
    CompareModelCells.spike_latency = PostModelCellFeatures.spike_latency - PreModelCellFeatures.spike_latency;
    CompareModelCells.inter_spike_interval = PostModelCellFeatures.inter_spike_interval ./ PreModelCellFeatures.inter_spike_interval;
...
function [V1_Pre,V2_Pre,V3_Pre, V1_Post,V2_Post,V3_Post] = DoubleRz_MultiComp(Iinj,dt, gCompFactor, gGCoupFactor, gNFactor, gKFactor, gAFactor, gLFactor, gHFactor, EL,EH, EN, EK, c1_factor, c2_factor, c3_factor)
    ...
    V1_Post = zeros(1,Ntotal); % [mV] membrane potential of somatic compartment of Presynaptic cell
    V2_Post = zeros(1,Ntotal); % [mV] membrane potential of Gap Junction compartment of Presynaptic cell
    V3_Post = zeros(1,Ntotal); % [mV] membrane potential of spike-initiation zone of Presynaptic cell
    ...
    for j = 1:Ntotal-1
        ...
        IGap = gGCoup     * ((V2_Pre(j)-V2_Post(j))) ; % current from Gap Junction
        ...
        dv2_dt_Pre = ( IC2_Pre - IC3_Pre - IGap) / c2_Pre; % -IGap
        ...
        dv2_dt_Post = ( IC2_Post - IC3_Post + IGap) / c2_Post; % +IGap
